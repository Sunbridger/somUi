<template>
    <div class="som-carousel">
        <div class="som-swiper" :style="{height: xheight}">
            <slot></slot>

            <div class="som-carousel-item"
                ...
                <a href="javascript:">
                    <div class="som-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
                    <p class="som-carousel-desc" v-if="showDescMask">{{ item.title }}</p>
                </a>
            </div>

            <div v-if="listTwoLoopItem.length > 0"
                class="som-carousel-item som-carousel-item-clone"
        ...
        <div :class="[dotsClass, 'som-indicator', `som-indicator-${dotsPosition}`]" v-show="showDots">
            <a href="javascript:" v-for="(key, index) in length" :key="index">
                <i class="som-icon-dot" :class="{'active': key - 1 === current}"></i>
            </a>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        render (index = 0) {
            ...
            this.swiper.on('swiped', (prev, index) => {
        ...
        getHeight () {
            ...
            if (!hasHeight) {
                if (this.aspectRatio) {
                    return this.$el.offsetWidth * this.aspectRatio + 'px';//eslint-disable-line
                }
                return '180px';
            }
        }
