import objectAssign from 'som-ui/src/utils/merge';
import SoCheckbox from '@souche-ui/so-ui/lib/checkbox';
...
const defaults = {
    ...
    selection: {
        ...
        realWidth: 48,
        order: '',
        className: 'som-table-column--selection'
    },
    expand: {
        width: 48,
        minWidth: 48,
...
const forced = {
    selection: {
        renderHeader: function(h, {column}) {
            if (this.store.states.selection.length && !this.isAllSelected && column.indeterminate) {
                return <so-checkbox
                    indeterminate
    ...
    expand: {
        ...
        renderCell: function(h, { row, store }, proxy) {
            ...
            return <div class={ 'som-table__expand-icon ' + (expanded ? 'som-table__expand-icon--expanded' : '') }
                  on-click={ () => proxy.handleExpandClick(row) }>
        <i class='som-icon som-icon-arrow-right'></i>
      </div>;
        },
        sortable: false,
        resizable: false,
        className: 'som-table__expand-column'
    }
};
...
export default {
    ...
    created() {
        ...
        ? <div class="cell som-tooltip" style={'width:' + ((data.column.realWidth || data.column.width) - 32) + 'px'}>{ renderCell(h, data) }</div>
        : <div class={[data.column.ellipsis && 'ellipsis', 'cell']}>{ renderCell(h, data) }</div>;
        };
    },
    ...
    mounted() {
        ...
        if (this.$scopedSlots.header) {
            if (this.type === 'selection') {
                console.warn('[SomUI Warn][TableColumn]Selection column doesn\'t allow to set scoped-slot header.');
            } else {
